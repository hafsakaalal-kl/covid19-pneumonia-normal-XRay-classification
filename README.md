...
2. Architectures
Transfer Learning : Utilisation de modèles pré‑entraînés sur ImageNet (VGG16, DenseNet121, ResNet50), avec fine‑tuning des dernières couches.

CNN personnalisé : Architecture légère à 5 blocs convolutionnels, avec normalisation par batch et dropout.

3. Entraînement
Optimiseur : Adam (taux d’apprentissage initial = 1e‑4)
...
Performances par classe (sur l’ensemble de test)
...
COVID-19 → Normal : 14 erreurs (53,8 % des erreurs)
...
Interprétation : La confusion la plus fréquente est entre COVID-19 et Normal, suggérant que certaines radiographies COVID-19 présentent des signes subtils ou proches de la normale.

🎯 Points Forts du Projet
Comparaison systématique de 4 architectures avec les mêmes données et protocole
...
Visualisations complètes (matrices de confusion, courbes d’apprentissage, etc.)

Sauvegarde des modèles et résultats pour reproductibilité

⚠️ Limitations et Perspectives
Limitations actuelles
Déséquilibre des classes (surtout entre « Normal » et « Pneumonie Virale »)

Taille d’image réduite pour des raisons computationnelles

Absence de validation croisée (un seul split train/val/test)

Manque d’interprétabilité (pas de cartes d’activation type Grad‑CAM)

Améliorations envisageables
Collecte de données supplémentaires pour équilibrer les classes

Augmentation plus poussée (mixup, cutmix, style transfer)

Ensemble de modèles (fusion des prédictions de plusieurs architectures)

Explicabilité : ajout de visualisations des régions d’intérêt

Déploiement sous forme d’API ou d’interface web pour tests en conditions réelles


🚀 Utilisation Pratique
Charger le modèle ResNet50 pré‑entraîné

Prétraiter une nouvelle radiographie (redimensionnement, normalisation)

Effectuer la prédiction et obtenir la classe ainsi qu’un score de confiance


Note : Ce modèle est conçu comme une aide au diagnostic et ne doit en aucun cas remplacer l’expertise d’un médecin ou radiologue.

✅ Conclusion
Le modèle ResNet50 atteint une performance exceptionnelle (98,29 % d’accuracy) sur la tâche de classification triple, surpassant les autres architectures testées. Ce projet démontre l’efficacité du transfer learning dans le domaine médical, même avec un jeu de données relativement limité et déséquilibré.

Les résultats sont prometteurs pour une utilisation en milieu clinique comme outil de triage rapide ou d’assistance au diagnostic, notamment dans des contextes où l’accès à des radiologues experts est limité.
